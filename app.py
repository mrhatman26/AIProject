...
def ai_process(skills_data):
    ...
    if skill_data is not None and len(skill_data) >= 1: #Make sure there is actually text in the skill list.
        ...
        ai_model_file.close()
        list_order_file = open("./static/Model/list_order.txt", "r") #This file is generated by model_creator.py. It is the needed order of the skills to work with the A.I.
        ai_skill_data = []
        skill_found = False
        for column in list_order_file: #This for loop creates the list needed for the A.I based off of the list_order_file.
            #If one of the given skills is the same as the one as in the file, a 1 is added to the ai_skill_list.
            #else a 0 is added instead.
            #1 = they have that skill and 0 = they don't have that skill.
            skill_found = False
            column = column.replace("-", " ")
            column = column.replace("_", " ")
            for skill in skill_data:
                print("Skill is " + skill, flush=True)
                if skill.lower() in column.lower(): #Don't forget the lower!!!!! You'll cry if you do... :(
                    ai_skill_data.append(1) #They have the skill
                    skill_found = True
            if skill_found is not True:
                ai_skill_data.append(0) #They do not have the skill
        print(ai_skill_data, flush=True)
        ai_skill_data = np.array(list(ai_skill_data), dtype=float) #Convert the list to a numpy array to work with the A.I
        ai_skill_data = ai_skill_data.reshape(1, -1) #Reformat the list to be in the required shape for the A.I
        out = ai_model.predict(X=ai_skill_data) #Give the list to the A.I and have it predict the job type.
        ...
        out_mapped = list(career_map.keys())[list(career_map.values()).index(out)] #Oh god...
        #The A.I only gives us a number. To get the actual result we need to use this map again. The above command takes the number
        #the A.I gives and uses it as the value in the map dictionary and retrieves the value's key which represents the job type.
        print(out_mapped)
        return redirect('/job_search/data=' + out_mapped) #Redirect to the job search page, but give it the A.I's answer too.
    else:
        return redirect('/') #If something goes wrong, just redirect to the home page.
